...
# Debian/Ubuntu
sudo apt install pandoc libpango-1.0-0 libpangoft2-1.0-0 libgdk-pixbuf-2.0-0
```

> WeasyPrint(HTML → PDF 렌더용)가 cairo/pango 시스템 라이브러리를 import time에 요구.
...
- **시스템**: Python 3.10+, pandoc, cairo·pango·gdk-pixbuf·libffi (WeasyPrint용)
- **`.venv` 안**: PyMuPDF, weasyprint, docling (`setup.sh`가 자동 설치)
